...
sudo nmap -Pn -vvv --disable-arp --reason -f -D 8.8.8.8 -sSV -O -sC -p 80,443 -oN "$domain/nmapDefault" $domain
sudo nmap -Pn -vvv --disable-arp --reason -f -D 8.8.8.8 -sSV -O -p 80,443 --script safe,version,vuln -oN "$domain/nmapSafe-Version-Vuln" $domain
sudo nmap -Pn -vvv --disable-arp --reason -f -D 8.8.8.8 -sSV -p 80,443 -oN "$domain/nmapFullHTTP" --script http-backup-finder,http-config-backup,http-errors,http-headers,http-iis-webdav-vuln,http-internal-ip-disclosure,http-methods,http-php-version,http-qnap-nas-info,http-robots.txt,http-shellshock,http-slowloris-check,http-waf-detect,"http-vuln*" $domain
